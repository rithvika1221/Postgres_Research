#!/usr/bin/env python3
"""
Subscriber Replication Monitor — 1 User, Variable Throughput
...
# Publisher connection for cross-VM LSN comparison
PUBLISHER_CONFIG = {
    "host": "172.16.0.4",
    "port": 5432,
    "dbname": "pub",
...
def get_tps_phase_from_time(elapsed_seconds):
    """
    Determine the current TPS phase based on elapsed time since start.
    Since this runs on the subscriber VM (no access to status file),
...
def get_publisher_lsn():
    """Connect to publisher and get current WAL LSN for lag comparison."""
    try:
...
def lsn_to_int(lsn_str):
    """Convert PostgreSQL LSN string (e.g., '0/16B3748') to integer bytes."""
...
SELECT
    ...
    s.last_msg_send_time::text      AS last_msg_send_time,
    s.last_msg_receipt_time::text   AS last_msg_receipt_time,
    EXTRACT(EPOCH FROM (
        now() - s.last_msg_receipt_time
    ))::numeric(10,3)               AS seconds_since_last_msg,
    (SELECT count(*) FROM ingest_data) AS subscriber_row_count
...
SELECT
    ...
    s.last_msg_send_time::text,
    s.last_msg_receipt_time::text,
    a.state                          AS worker_state,
    a.wait_event_type,
    a.wait_event,
    a.query                          AS current_query,
    EXTRACT(EPOCH FROM (now() - a.query_start))::numeric(10,3)
                                     AS query_runtime_sec
FROM pg_stat_subscription s
...
def main():
    parser = argparse.ArgumentParser(
        description="Subscriber monitor for 1-user variable TPS experiment"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-publisher-check", action="store_true",
        help="Skip cross-VM publisher LSN comparison"
    )
    ...
    output_filename = f"subscriber_metrics_1user_varTPS_{timestamp}.xlsx"
    output_path = os.path.join(args.output_dir, output_filename)

    print("=" * 65)
    print("  Subscriber Replication Monitor — Variable TPS")
    ...
    try:
        ...
        sub = cur.fetchone()
        if sub:
            print(f"  Subscription '{sub[0]}' exists.")
        else:
            print("  WARNING: Subscription 'mysub' not found!")
        conn.commit()
    except Exception as e:
        print(f"  WARNING: {e}")
        conn.rollback()
    ...
    if not args.no_publisher_check:
        ...
        if pub_lsn:
            print(f"  Publisher reachable. Current WAL LSN: {pub_lsn}")
        else:
            print("  WARNING: Cannot reach publisher.")
    else:
        print("[3/3] Skipping publisher connectivity check.")
    ...
    print(f"  IMPORTANT: Start this BEFORE the load generator on the publisher!")
    print(f"  Both clocks must be roughly in sync for TPS phase to match.\n")
