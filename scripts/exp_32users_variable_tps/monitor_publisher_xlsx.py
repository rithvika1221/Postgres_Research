#!/usr/bin/env python3
"""
Publisher Replication Monitor — 32 users (-c 32 -j 16), Variable Throughput
...
def get_current_tps_phase(elapsed):
    name, tps = get_tps_phase_from_file()
...
QUERIES = [
    ("Replication_Lag",
     ...
     ["Sample Time","TPS Phase","Target TPS","PID","User","Application","Client Addr","State","Sent LSN","Write LSN","Flush LSN","Replay LSN","Write Lag (bytes)","Flush Lag (bytes)","Replay Lag (bytes)","Write Lag (time)","Flush Lag (time)","Replay Lag (time)","Sync State"]),
    ("WAL_Stats",
     ...
     ["Sample Time","TPS Phase","Target TPS","Current WAL LSN","Current WAL Insert LSN","Insert Ahead (bytes)","WAL File Count","Total WAL Size"]),
    ("Database_Stats",
     ...
     ["Sample Time","TPS Phase","Target TPS","Backends","Commits","Rollbacks","Blocks Read","Blocks Hit","Tuples Returned","Tuples Fetched","Tuples Inserted","Tuples Updated","Tuples Deleted","Conflicts","Temp Files","Temp Bytes","Deadlocks","Block Read Time (ms)","Block Write Time (ms)"]),
    ("Connections",
     ...
     ["Sample Time","TPS Phase","Target TPS","State","Wait Event Type","Wait Event","Connection Count","Applications"]),
    ("Replication_Slots",
     ...
     ["Sample Time","TPS Phase","Target TPS","Slot Name","Plugin","Slot Type","Active","Active PID","Restart LSN","Confirmed Flush LSN","Retained WAL (bytes)","Retained WAL (pretty)","WAL Status"]),
]
...
def collect_sample(conn, sheet_data, elapsed):
    cur = conn.cursor()
    phase_name, target_tps = get_current_tps_phase(elapsed)
    for i, (_, query, headers) in enumerate(QUERIES):
        try:
            ...
            for row in rows:
                ...
                for val in row:
                    if first:
                        ...
                        processed.append(phase_name); processed.append(target_tps); first = False; continue
            ...
            if not rows:
                r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), phase_name, target_tps, "NO DATA"]
                r.extend([""] * (len(headers) - 4)); sheet_data[i].append(r)
        except Exception as e:
            r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), phase_name, target_tps, f"ERROR: {e}"]
...
def main():
    ...
    outfile = f"publisher_metrics_32users_varTPS_{ts}.xlsx"
    outpath = os.path.join(args.output_dir, outfile)

    print("=" * 65)
    print("  Publisher Monitor — 32 users (-c 32 -j 16), Variable TPS")
    print("=" * 65)
    ...
    try:
        while time.time() < end:
            ...
            print(f"  Sample {count:>4} | {int(elapsed):>4}s | {pn:25s} | Lag: {lag:>15s} | Left: {max(0, int(end - time.time()))}s")
