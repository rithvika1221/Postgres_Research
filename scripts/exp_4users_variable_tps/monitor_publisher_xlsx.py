#!/usr/bin/env python3
"""
Publisher Replication Monitor — 4 Users, Variable Throughput
...
Each row includes "TPS Phase" and "Target TPS" columns so you can
correlate metrics with the throughput level at that point in time.
...
SAMPLE_INTERVAL = 5
DURATION_SECONDS = 660     # 11 min (10-min experiment + 1-min buffer)
OUTPUT_DIR = "."
...
SELECT
    now()                                    AS sample_time,
    pid, usename, application_name, client_addr::text, state,
    sent_lsn::text, write_lsn::text, flush_lsn::text, replay_lsn::text,
    pg_wal_lsn_diff(sent_lsn, write_lsn)    AS write_lag_bytes,
    pg_wal_lsn_diff(sent_lsn, flush_lsn)    AS flush_lag_bytes,
    pg_wal_lsn_diff(sent_lsn, replay_lsn)   AS replay_lag_bytes,
    write_lag::text, flush_lag::text, replay_lag::text, sync_state
...
SELECT
    ...
    pg_wal_lsn_diff(pg_current_wal_insert_lsn(), pg_current_wal_lsn()),
...
FROM pg_stat_database WHERE datname = 'pub';
"""
...
FROM pg_stat_activity WHERE datname = 'pub'
...
def main():
    parser = argparse.ArgumentParser(
        description="Publisher monitor for 4-user variable TPS experiment"
    )
    ...
    output_filename = f"publisher_metrics_4users_varTPS_{timestamp}.xlsx"
    output_path = os.path.join(args.output_dir, output_filename)

    print("=" * 65)
    print("  Publisher Replication Monitor — 4 Users, Variable TPS")
