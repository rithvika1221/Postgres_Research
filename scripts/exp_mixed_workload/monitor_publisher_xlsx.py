#!/usr/bin/env python3
"""
Publisher Replication Monitor — Mixed Workload Experiment
Each row includes "TPS Phase", "Target TPS", "Workers", and "Workload Mix" columns.

Phase Schedule (2 min each, 12 min total):
  Phase 1: 1 user,  100 TPS, INSERT only
  Phase 2: 4 users, 500 TPS, INSERT only
  Phase 3: 8 users, 1000 TPS, 70/20/10
  Phase 4: 16 users, 2500 TPS, 70/20/10
  Phase 5: 32 users, 5000 TPS, 70/20/10
  Phase 6: 64 users, 5000 TPS, 50/30/20
...
TPS_SCHEDULE = [
    ("Phase 1: 1 user, 100 TPS, INSERT only",    100,  1, "INSERT only", 120),
    ("Phase 2: 4 users, 500 TPS, INSERT only",   500,  4, "INSERT only", 120),
    ("Phase 3: 8 users, 1000 TPS, 70/20/10",    1000,  8, "70/20/10",    120),
    ("Phase 4: 16 users, 2500 TPS, 70/20/10",   2500, 16, "70/20/10",    120),
    ("Phase 5: 32 users, 5000 TPS, 70/20/10",   5000, 32, "70/20/10",    120),
    ("Phase 6: 64 users, 5000 TPS, 50/30/20",   5000, 64, "50/30/20",    120),
]
...
def get_tps_phase_from_file():
    try:
        if os.path.exists(STATUS_FILE):
            ...
            target_tps = 0; workers = 0; mix = "Unknown"
            for line in lines:
                ...
                elif line.startswith("workload_mix="): mix = line.split("=")[1].strip()
            return phase_name, target_tps, workers, mix
    except Exception:
        pass
...
def get_tps_phase_from_time(elapsed):
    cumulative = 0
    for name, tps, w, mix, dur in TPS_SCHEDULE:
        if elapsed < cumulative + dur:
            return name, tps, w, mix
        cumulative += dur
    return "Completed", 0, 0, ""

def get_current_tps_phase(elapsed):
    name, tps, w, mix = get_tps_phase_from_file()
    if name == "Unknown" or tps == 0:
        name, tps, w, mix = get_tps_phase_from_time(elapsed)
    return name, tps, w, mix
...
QUERIES = [
    ("Replication_Lag",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Workload Mix","PID","User","Application","Client Addr","State","Sent LSN","Write LSN","Flush LSN","Replay LSN","Write Lag (bytes)","Flush Lag (bytes)","Replay Lag (bytes)","Write Lag (time)","Flush Lag (time)","Replay Lag (time)","Sync State"]),
    ("WAL_Stats",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Workload Mix","Current WAL LSN","Current WAL Insert LSN","Insert Ahead (bytes)","WAL File Count","Total WAL Size"]),
    ("Database_Stats",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Workload Mix","Backends","Commits","Rollbacks","Blocks Read","Blocks Hit","Tuples Returned","Tuples Fetched","Tuples Inserted","Tuples Updated","Tuples Deleted","Conflicts","Temp Files","Temp Bytes","Deadlocks","Block Read Time (ms)","Block Write Time (ms)"]),
    ("Connections",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Workload Mix","State","Wait Event Type","Wait Event","Connection Count","Applications"]),
    ("Replication_Slots",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Workload Mix","Slot Name","Plugin","Slot Type","Active","Active PID","Restart LSN","Confirmed Flush LSN","Retained WAL (bytes)","Retained WAL (pretty)","WAL Status"]),
]
...
def collect_sample(conn, sheet_data, elapsed):
    cur = conn.cursor()
    phase_name, target_tps, workers, mix = get_current_tps_phase(elapsed)
    for i, (_, query, headers) in enumerate(QUERIES):
        try:
            ...
            for row in rows:
                ...
                for val in row:
                    if first:
                        ...
                        processed.append(workers); processed.append(mix)
            ...
            if not rows:
                r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), phase_name, target_tps, workers, mix, "NO DATA"]
                r.extend([""] * (len(headers) - 6)); sheet_data[i].append(r)
        except Exception as e:
            r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), phase_name, target_tps, workers, mix, f"ERROR: {e}"]
...
def main():
    ...
    outfile = f"publisher_metrics_mixed_{ts}.xlsx"
    outpath = os.path.join(args.output_dir, outfile)

    print("=" * 65)
    print("  Publisher Monitor — Mixed Workload Experiment")
    print("=" * 65)
    ...
    for name, tps, w, mix, dur in TPS_SCHEDULE:
        ...
        print(f"    {m1}:{s1:02d}-{m2}:{s2:02d} -> {tps:>5,} TPS | {w:>2} workers | {mix}")
        cumulative += dur
    print("=" * 65)
