#!/usr/bin/env python3
"""
Subscriber Replication Monitor — Mixed Workload Experiment
...
def get_tps_phase(elapsed):
    ...
    for name, tps, w, mix, dur in TPS_SCHEDULE:
        if elapsed < cumulative + dur: return name, tps, w, mix
        cumulative += dur
...
QUERIES = [
    ("Subscription_Lag",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Workload Mix","Subscription","Worker PID","Received LSN","Latest End LSN","Last Msg Send Time","Last Msg Receipt Time","Seconds Since Last Msg","Subscriber Row Count"]),
    ("Apply_Stats",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Workload Mix","Subscription","PID","Received LSN","Latest End LSN","Last Msg Send Time","Last Msg Receipt Time","Worker State","Wait Event Type","Wait Event","Current Query","Query Runtime (sec)"]),
    ("Worker_Activity",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Workload Mix","PID","User","Application","State","Wait Event Type","Wait Event","Backend Type","Query","Query Runtime (sec)"]),
    ("Table_Status",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Workload Mix","Subscription OID","Subscription Name","Table Name","Sync State","Sync LSN"]),
]
...
def collect_sample(conn, sheet_data, elapsed):
    cur = conn.cursor()
    pn, tt, w, mix = get_tps_phase(elapsed)
    for i, (_, query, headers) in enumerate(QUERIES):
        try:
            ...
            for row in rows:
                ...
                for val in row:
                    if first:
                        ...
                        processed.append(pn); processed.append(tt)
                        processed.append(w); processed.append(mix)
            ...
            if not rows:
                r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), pn, tt, w, mix, "NO DATA"]
                r.extend([""] * (len(headers) - 6)); sheet_data[i].append(r)
        except Exception as e:
            r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), pn, tt, w, mix, f"ERROR: {e}"]
...
def main():
    ...
    outfile = f"subscriber_metrics_mixed_{ts}.xlsx"
    outpath = os.path.join(args.output_dir, outfile)

    print("=" * 65)
    print("  Subscriber Monitor — Mixed Workload Experiment")
    print("=" * 65)
    ...
    try:
        while time.time() < end:
            ...
            print(f"  {datetime.now().strftime('%H:%M:%S')} | Sample {count:>4} | {int(elapsed):>4}s | {pn:45s} | Rows: {rc:>10} | Lag: {lag:>15s} | Left: {max(0, int(end - time.time()))}s")
