#!/usr/bin/env python3
"""
Subscriber Replication Monitor — Network Latency Experiment
...
Usage:   python monitor_subscriber_xlsx.py --latency 50ms
"""
...
QUERIES = [
    ("Subscription_Lag",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Network Latency","Subscription","Worker PID","Received LSN","Latest End LSN","Last Msg Send Time","Last Msg Receipt Time","Seconds Since Last Msg","Subscriber Row Count"]),
    ("Apply_Stats",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Network Latency","Subscription","PID","Received LSN","Latest End LSN","Last Msg Send Time","Last Msg Receipt Time","Worker State","Wait Event Type","Wait Event","Current Query","Query Runtime (sec)"]),
    ("Worker_Activity",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Network Latency","PID","User","Application","State","Wait Event Type","Wait Event","Backend Type","Query","Query Runtime (sec)"]),
    ("Table_Status",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Network Latency","Subscription OID","Subscription Name","Table Name","Sync State","Sync LSN"]),
]
...
def collect_sample(conn, sheet_data, elapsed, latency_label, workers):
    cur = conn.cursor()
    ...
    for i, (_, query, headers) in enumerate(QUERIES):
        try:
            ...
            for row in rows:
                ...
                for val in row:
                    if first:
                        ...
                        processed.append(workers); processed.append(latency_label)
            ...
            if not rows:
                r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), pn, tt, workers, latency_label, "NO DATA"]
                r.extend([""] * (len(headers) - 6)); sheet_data[i].append(r)
        except Exception as e:
            r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), pn, tt, workers, latency_label, f"ERROR: {e}"]
...
def main():
    ...
    latency_label = args.latency; num_workers = args.workers

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    outfile = f"subscriber_metrics_latency_{latency_label}_{ts}.xlsx"
    outpath = os.path.join(args.output_dir, outfile)

    print("=" * 65)
    print("  Subscriber Monitor — Network Latency Experiment")
    print("=" * 65)
    ...
    try:
        while time.time() < end:
            ...
            pn, tt = collect_sample(conn, sheet_data, elapsed, latency_label, num_workers); count += 1
            ...
            print(f"  {datetime.now().strftime('%H:%M:%S')} | Sample {count:>4} | {int(elapsed):>4}s | {pn:45s} | Latency: {latency_label:>5s} | Rows: {rc:>10} | Lag: {lag:>15s} | Left: {max(0, int(end - time.time()))}s")
