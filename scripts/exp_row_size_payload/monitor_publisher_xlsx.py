#!/usr/bin/env python3
"""
Publisher Replication Monitor — Row Size Payload Experiment
Each row includes "TPS Phase", "Target TPS", "Workers", and "Payload Size" columns.
...
TPS_SCHEDULE = [
    ("Phase 1: 4 users, 500 TPS, 10KB",     500,   4, "10KB",  120),
    ("Phase 2: 16 users, 1000 TPS, 50KB",  1000,  16, "50KB",  120),
    ("Phase 3: 32 users, 2500 TPS, 100KB", 2500,  32, "100KB", 120),
    ("Phase 4: 64 users, 5000 TPS, 250KB", 5000,  64, "250KB", 120),
    ("Phase 5: 64 users, 5000 TPS, 500KB", 5000,  64, "500KB", 120),
    ("Phase 6: 64 users, 5000 TPS, 1MB",   5000,  64, "1MB",   120),
]
...
def get_tps_phase_from_file():
    try:
        if os.path.exists(STATUS_FILE):
            ...
            target_tps = 0; workers = 0; payload = "Unknown"
            for line in lines:
                ...
                elif line.startswith("payload_size="): payload = line.split("=")[1].strip()
            return phase_name, target_tps, workers, payload
    except Exception:
        pass
...
def get_tps_phase_from_time(elapsed):
    cumulative = 0
    for name, tps, w, payload, dur in TPS_SCHEDULE:
        if elapsed < cumulative + dur:
            return name, tps, w, payload
        cumulative += dur
    return "Completed", 0, 0, ""

def get_current_tps_phase(elapsed):
    name, tps, w, payload = get_tps_phase_from_file()
    if name == "Unknown" or tps == 0:
        name, tps, w, payload = get_tps_phase_from_time(elapsed)
    return name, tps, w, payload
...
QUERIES = [
    ("Replication_Lag",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Payload Size","PID","User","Application","Client Addr","State","Sent LSN","Write LSN","Flush LSN","Replay LSN","Write Lag (bytes)","Flush Lag (bytes)","Replay Lag (bytes)","Write Lag (time)","Flush Lag (time)","Replay Lag (time)","Sync State"]),
    ("WAL_Stats",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Payload Size","Current WAL LSN","Current WAL Insert LSN","Insert Ahead (bytes)","WAL File Count","Total WAL Size"]),
    ("Database_Stats",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Payload Size","Backends","Commits","Rollbacks","Blocks Read","Blocks Hit","Tuples Returned","Tuples Fetched","Tuples Inserted","Tuples Updated","Tuples Deleted","Conflicts","Temp Files","Temp Bytes","Deadlocks","Block Read Time (ms)","Block Write Time (ms)"]),
    ("Connections",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Payload Size","State","Wait Event Type","Wait Event","Connection Count","Applications"]),
    ("Replication_Slots",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Payload Size","Slot Name","Plugin","Slot Type","Active","Active PID","Restart LSN","Confirmed Flush LSN","Retained WAL (bytes)","Retained WAL (pretty)","WAL Status"]),
]
...
def collect_sample(conn, sheet_data, elapsed):
    cur = conn.cursor()
    phase_name, target_tps, workers, payload = get_current_tps_phase(elapsed)
    for i, (_, query, headers) in enumerate(QUERIES):
        try:
            ...
            for row in rows:
                ...
                for val in row:
                    if first:
                        ...
                        processed.append(workers); processed.append(payload)
            ...
            if not rows:
                r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), phase_name, target_tps, workers, payload, "NO DATA"]
                r.extend([""] * (len(headers) - 6)); sheet_data[i].append(r)
        except Exception as e:
            r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), phase_name, target_tps, workers, payload, f"ERROR: {e}"]
...
def main():
    ...
    outfile = f"publisher_metrics_rowsize_{ts}.xlsx"
    outpath = os.path.join(args.output_dir, outfile)

    print("=" * 65)
    print("  Publisher Monitor — Row Size Payload Experiment")
    print("=" * 65)
    ...
    for name, tps, w, payload, dur in TPS_SCHEDULE:
        ...
        print(f"    {m1}:{s1:02d}-{m2}:{s2:02d} -> {tps:>5,} TPS | {w:>2} workers | {payload}")
        cumulative += dur
    print("=" * 65)
