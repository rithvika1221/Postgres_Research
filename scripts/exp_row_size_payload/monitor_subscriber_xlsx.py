#!/usr/bin/env python3
"""
Subscriber Replication Monitor — Row Size Payload Experiment
...
def get_tps_phase(elapsed):
    ...
    for name, tps, w, payload, dur in TPS_SCHEDULE:
        if elapsed < cumulative + dur: return name, tps, w, payload
        cumulative += dur
...
QUERIES = [
    ("Subscription_Lag",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Payload Size","Subscription","Worker PID","Received LSN","Latest End LSN","Last Msg Send Time","Last Msg Receipt Time","Seconds Since Last Msg","Subscriber Row Count"]),
    ("Apply_Stats",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Payload Size","Subscription","PID","Received LSN","Latest End LSN","Last Msg Send Time","Last Msg Receipt Time","Worker State","Wait Event Type","Wait Event","Current Query","Query Runtime (sec)"]),
    ("Worker_Activity",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Payload Size","PID","User","Application","State","Wait Event Type","Wait Event","Backend Type","Query","Query Runtime (sec)"]),
    ("Table_Status",
     ...
     ["Sample Time","TPS Phase","Target TPS","Workers","Payload Size","Subscription OID","Subscription Name","Table Name","Sync State","Sync LSN"]),
]
...
def collect_sample(conn, sheet_data, elapsed):
    cur = conn.cursor()
    pn, tt, w, payload = get_tps_phase(elapsed)
    for i, (_, query, headers) in enumerate(QUERIES):
        try:
            ...
            for row in rows:
                ...
                for val in row:
                    if first:
                        ...
                        processed.append(w); processed.append(payload)
            ...
            if not rows:
                r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), pn, tt, w, payload, "NO DATA"]
                r.extend([""] * (len(headers) - 6)); sheet_data[i].append(r)
        except Exception as e:
            r = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), pn, tt, w, payload, f"ERROR: {e}"]
...
def main():
    ...
    outfile = f"subscriber_metrics_rowsize_{ts}.xlsx"
    outpath = os.path.join(args.output_dir, outfile)

    print("=" * 65)
    print("  Subscriber Monitor — Row Size Payload Experiment")
    print("=" * 65)
